...
void RasterFont::printMultilineText(const std::string& text, float start_x, float start_y,
                                    int bbWidth, int bbHeight, u32 color, float scale)
{
  std::vector<GLfloat> vertices(text.length() * 6 * 4);

  int usage = 0;
  GLfloat delta_x = scale * CHARACTER_WIDTH / GLfloat(bbWidth);
  GLfloat delta_y = scale * CHARACTER_HEIGHT / GLfloat(bbHeight);
