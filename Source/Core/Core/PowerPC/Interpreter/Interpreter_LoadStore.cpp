...
void Interpreter::lwarx(UGeckoInstruction inst)
{
  ...
  {
    rGPR[inst.RD] = temp;
    PowerPC::interpreter.m_reserve = true;
    PowerPC::interpreter.m_reserve_address = address;
  }
}
...
void Interpreter::stwcxd(UGeckoInstruction inst)
{
  ...
  if (PowerPC::interpreter.m_reserve)
  {
    if (address == PowerPC::interpreter.m_reserve_address)
    {
      PowerPC::Write_U32(rGPR[inst.RS], address);
      if (!(PowerPC::ppcState.Exceptions & EXCEPTION_DSI))
      {
        PowerPC::interpreter.m_reserve = false;
