...
public:
  ...
  ~Mapping()
  {
    for(int i = 0; i < ArraySize(m_read8_handlers); ++i)
    {
      if(m_read8_handlers[i])
        delete m_read8_handlers[i];
    }

    for(int i = 0; i < ArraySize(m_read16_handlers); ++i)
    {
      if(m_read16_handlers[i])
        delete m_read16_handlers[i];
    }

    for(int i = 0; i < ArraySize(m_read32_handlers); ++i)
    {
      if(m_read32_handlers[i])
        delete m_read32_handlers[i];
    }

    for(int i = 0; i < ArraySize(m_write8_handlers); ++i)
    {
      if(m_write8_handlers[i])
        delete m_write8_handlers[i];
    }

    for(int i = 0; i < ArraySize(m_write16_handlers); ++i)
    {
      if(m_write16_handlers[i])
        delete m_write16_handlers[i];
    }

    for(int i = 0; i < ArraySize(m_write32_handlers); ++i)
    {
...
private:
  ...
  // accesses).
  ReadHandler<u8>* m_read8_handlers[NUM_MMIOS / sizeof(u8)];
  ReadHandler<u16>* m_read16_handlers[NUM_MMIOS / sizeof(u16)];
  ReadHandler<u32>* m_read32_handlers[NUM_MMIOS / sizeof(u32)];

  WriteHandler<u8>* m_write8_handlers[NUM_MMIOS / sizeof(u8)];
  WriteHandler<u16>* m_write16_handlers[NUM_MMIOS / sizeof(u16)];
  WriteHandler<u32>* m_write32_handlers[NUM_MMIOS / sizeof(u32)];
};
